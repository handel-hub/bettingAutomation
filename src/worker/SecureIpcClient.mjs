...
import { createExecutionEnvelope, validateExecutionEnvelope, ExecutionMessageType } from './protocol.mjs';

/**
 * Secure IPC Client that connects the Execution Worker to the Control Plane Windows Named Pipe.
...
export class SecureIpcClient extends EventEmitter {
  ...
  sendEnvelope(type, payload, traceId) {
    const envelope = createExecutionEnvelope(type, payload, traceId);
    return this.send(envelope);
  }

  /**
   * Raw length-prefixed send.
   * @param {any} message
   */
  send(message) {
   ...
   * @param {function(): { activeBrowsers?: number, engineStatus?: string }} [telemetryProvider]
   */
  ...
  _handleData(chunk) {
    ...
    while (this._rxBuffer.length >= 4) {
      ...
      try {
        ...
        const { valid, envelope, error } = validateExecutionEnvelope(rawEnvelope);
        if (valid) {
          this.emit('envelope', envelope);
          this.emit('message', envelope); // Backwards compatibility
        } else {
          this.emit('message', rawEnvelope); // Raw payload fallback
        }
      } catch (e) {
