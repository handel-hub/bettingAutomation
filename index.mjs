...
import { ProcessLifecycleManager } from './src/ProcessLifecycleManager.mjs';
...
async function main() {
    ...
    // Initialize Centralized Panic Supervisor
    const lifecycleManager = new ProcessLifecycleManager(logger, controller);

    try {
        await controller.start();
    } catch (err) {
        logger.error({ err }, `Fatal error during startup: ${err.message}`);
        await lifecycleManager.shutdown('StartupError');
    }
}
